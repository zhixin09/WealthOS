                    ...
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-2 shadow-lg">
                          <p className="text-xs font-medium text-zinc-100">{item.category}</p>
                          <p className="text-sm font-bold text-zinc-50 font-mono">
                            ${item.value.toLocaleString()}
                          </p>
                          <p className="text-[10px] text-zinc-500">{item.percentage}%</p>
                        </div>
                      );
                    }
          ...
          <div className="flex flex-1 flex-col gap-2">
            {data.map((item, index) => (
              <div key={item.category} className="flex items-center gap-2">
                ...
                />
                <span className="flex-1 text-xs text-zinc-400">{item.category}</span>
                <span className="text-xs font-medium text-zinc-100 font-mono">
                  {item.percentage}%
                </span>
              </div>
            ))}
